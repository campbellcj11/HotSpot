...
import webImage from '../images/globe.png'
import emailImage from '../images/mail.png'
import clockImage from '../images/time.png'
import pinImage from '../images/pin.png'
import dollarImage from '../images/money.png'
import infoImage from '../images/info.png'
import connectImage from '../images/share-android.png'
import linkImage from '../images/share-ios.png'
...
class EventPage extends Component {
  ...
  emailShare()
  {
    if(this.props.currentSelection.email_contact){
      var subject = 'HotSpot: ' + this.props.currentSelection.name;
      var recipient = this.props.currentSelection.email_contact;
      var body = 'I would like to know more about your event: ' + this.props.currentSelection.name;

      Linking.openURL('mailto:' + recipient + '?subject=' + subject + '&body=' + body);
    } else {
      Alert.alert('Unavailable', 'There is no email contact specified for this event.')
    }
  }
  ...
  render() {
    ...
    var startDate = new Date(this.props.currentSelection.start_date);

    dateMonth = months[startDate.getMonth()];
    dateNumber = startDate.getDate();
    dateYear = startDate.getUTCFullYear();
    dateHour = startDate.getHours();
    dateMinute = startDate.getMinutes();
    ...
    var diff = startDate - currentDate;
    ...
    let shareOptions = {
          title: this.props.currentSelection.name,
          message: this.props.currentSelection.short_description,
          url: this.props.currentSelection.website,
          subject: this.props.currentSelection.name //  for email
    };

    var arr = [];
    for(var i=0;i<this.props.currentSelection.tags.length;i++)
    {
      var tag = this.props.currentSelection.tags[i];
      arr.push(
          ...
          <View style={{marginTop:5}}>
            ...
            <BlankButton style={{marginBottom:20}} onPress={() => this.openCalendar()}>
              <View style={{flexDirection:'row',alignItems:'center'}}>
                <Image style={{marginLeft:16,marginRight:32,width:24,height:24,tintColor:appColors.DARK_GRAY}} source={clockImage}/>
            ...
            <BlankButton style={{marginBottom:20}} onPress={() => this.openMap()}>
              <View style={{flexDirection:'row',alignItems:'center',}}>
                <Image style={{marginLeft:16,marginRight:32,width:24,height:24,tintColor:appColors.DARK_GRAY}} source={pinImage}/>
                <View style={{marginRight:24}}>
                  <Text style={{fontFamily:appStyleVariables.SYSTEM_REGULAR_FONT,fontWeight:'bold',fontSize:15,color:'black'}}>{this.props.currentSelection.venue_name}</Text>
                  <Text style={{fontFamily:appStyleVariables.SYSTEM_REGULAR_FONT,fontWeight:'bold',fontSize:10.5,color:appColors.DARK_GRAY}}>{addressString}</Text>
                </View>
              </View>
            </BlankButton>

            <View style={{flexDirection:'row',alignItems:'center',marginBottom:20}}>
              <Image style={{marginLeft:16,marginRight:32,width:24,height:24,tintColor:appColors.DARK_GRAY}} source={dollarImage}/>
              <View style={{marginRight:24}}>
                <Text style={{fontFamily:appStyleVariables.SYSTEM_REGULAR_FONT,fontWeight:'bold',fontSize:15,color:'black'}}>$ {this.props.currentSelection.price}</Text>
              </View>
            </View>

            <View style={{flexDirection:'row',alignItems:'center',marginBottom:20}}>
              <View stlye={{flex:1}}>
                <Image style={{marginLeft:16,marginRight:32,width:24,height:24,tintColor:appColors.DARK_GRAY}} source={infoImage}/>
                <View style={{flex:2}}/>
              </View>
              <View style={{marginRight:24,flex:1}}>
                <Text numberOfLines={0} style={{fontFamily:appStyleVariables.SYSTEM_REGULAR_FONT,fontWeight:'bold',fontSize:12,lineHeight:16,color:appColors.DARK_GRAY}}>{this.props.currentSelection.long_description}</Text>
              </View>
            </View>
          </View>
        </View>
